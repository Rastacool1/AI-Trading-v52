...
def _normalize_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Obsługuje polskie i angielskie nagłówki, oraz fallback pozycyjny (kol.0=Date, kol.4=Close).
    """
    ...
    # Znajdź 'Close' w wariantach PL/EN
    close_col = None
    ...
    # Fallback pozycyjny dla typowego Stooq: Data;Otwarcie;Najwyzszy;Najnizszy;Zamkniecie;Wolumen
    ...
    if close_col is None or "Date" not in df.columns:
        # Kolejny fallback: jeśli 2+ kolumn i pierwsza wygląda na datę — potraktuj 0=Date, ostatnia=Close
        if df.shape[1] >= 2:
    ...
    # liczby mogą mieć przecinek dziesiętny
...
def _try_read_text(text: str) -> Optional[pd.DataFrame]:
    """Próby parsowania z tekstu: sniffer + kilka sep + engine='python' (autodetekcja)."""
    if not text:
        return None

    # 1) spróbuj autodetekcji engine='python' (sep=None)
    try:
...
def _manual_parse(text: str) -> Optional[pd.DataFrame]:
    """Ręczny parser: autodetekcja ;/,, tolerancja CRLF/BOM, wyrównanie długości wierszy."""
    if not text:
        return None
