...
def walk_forward(close: pd.Series, sentiment: pd.Series | None, space: dict, folds:int=4, cost_bps:int=10):
    # Podziel dane na sekwencyjne foldy IS/OS (purged: overlap wycięty)
    n = len(close)
    fold_size = n // (folds+1)
    results = []
    ...
    for f in range(folds):
        ...
        best = None
        for rsi_w, rsi_b, rsi_s, ma_f, ma_m, ma_s, bb_w, bb_std, wr, wm, wbb, watr, wbrk, ws, t_buy, t_sell, pm, pw in product(
            space["rsi_window"], space["rsi_buy"], space["rsi_sell"],
            space["ma_fast"], space["ma_mid"], space["ma_slow"],
            space["bb_window"], space["bb_std"],
            space["w_rsi"], space["w_ma"], space["w_bb"], space["w_atr"], space["w_breakout"], space["w_sent"],
            ...
            p = SignalParams(
                ...
                w_rsi=wr, w_ma=wm, w_bb=wbb, w_atr=watr, w_breakout=wbrk, w_sent=ws,
    ...
    # Stabilność parametrów: zbierz najczęściej wybierane zakresy
    stability = {}
    for r in results:
        p = r["params"]
